...
import (
	...
	"github.com/chagasVinicius/go-starter/kit/kafka/schemaregistry/cache"
)
...
*
* GET /subjects/{string: subject}/versions/{int|string('latest'): version}
*	returns: JSON blob *schemaMetadata*; raises: 404[01, 02], 422[02], 500[01]
* GET /subjects/{string: subject}/versions/{int|string('latest'): version}/schema returns:
*	JSON blob: schema(unescaped); raises: 404, 422, 500[01, 02, 03]
*
...
* -DELETE /subjects/{string: subject}/versions/{int|str('latest'): version} returns int:
*	deleted version id; raises: 404[01, 02]
*
...
* -POST /compatibility/subjects/{string: subject}/versions/{int:string('latest'): version} returns:
* 	JSON bool:is_compatible; raises: 404[01,02], 422[01,02], 500[01]
*
* ====SerializerConfig====
...
func NewClient(c Config) (Client, error) {
	...
	restService, err := newRestService(c)
	if err != nil {
		return nil, err
	}
	...
	if c.CacheCapacity != 0 {
		schemaCache, err = cache.NewLRUCache(c.CacheCapacity)
		if err != nil {
			return nil, err
		}
		idCache, err = cache.NewLRUCache(c.CacheCapacity)
		if err != nil {
			return nil, err
		}
		versionCache, err = cache.NewLRUCache(c.CacheCapacity)
		if err != nil {
			return nil, err
		}
	} else {
...
func NewMockClient() (Client, error) {
	return NewClient(NewConfig("mock://srtest"))
}

// Register registers Schema aliased with subject.
//
//nolint:dupl
func (c *client) Register(subject string, schema SchemaInfo, normalize bool) (int, error) {
	...
	if !ok {
		err = c.restService.handleRequest(
			newRequest("POST", versionNormalize, &metadata, url.PathEscape(subject), normalize),
			&metadata,
		)
		if err == nil {
...
func (c *client) GetBySubjectAndID(subject string, id int) (SchemaInfo, error) {
	cacheKey := subjectID{
		subject: subject,
		id:      id,
	}
...
func (c *client) GetID(subject string, schema SchemaInfo, normalize bool) (int, error) {
	return c.Register(subject, schema, normalize)
}

// GetLatestSchemaMetadata fetches latest version registered with the provided subject
// Returns SchemaMetadata object.
func (c *client) GetLatestSchemaMetadata(subject string) (SchemaMetadata, error) {
	var metadata SchemaMetadata

	req := newRequest("GET", versions, nil, url.PathEscape(subject), "latest")
	if err := c.restService.handleRequest(req, &metadata); err != nil {
...
// Returns SchemaMetadata object.
func (c *client) GetSchemaMetadata(subject string, version int) (SchemaMetadata, error) {
	var metadata SchemaMetadata

	req := newRequest("GET", versions, nil, url.PathEscape(subject), version)
	if err := c.restService.handleRequest(req, &metadata); err != nil {
		return SchemaMetadata{}, err
	}
...
func (c *client) GetAllVersions(subject string) ([]int, error) {
	var result []int

	req := newRequest("GET", version, nil, url.PathEscape(subject))
	if err := c.restService.handleRequest(req, &result); err != nil {
		return nil, err
	}
...
//
//nolint:dupl
func (c *client) GetVersion(subject string, schema SchemaInfo, normalize bool) (int, error) {
	schemaJSON, err := schema.MarshalJSON()
	if err != nil {
		return -1, err
	}
	...
	if !ok {
		err = c.restService.handleRequest(
			newRequest("POST", subjectsNormalize, &metadata, url.PathEscape(subject), normalize),
			&metadata,
		)
		if err == nil {
...
func (c *client) GetAllSubjects() ([]string, error) {
	...
	if err := c.restService.handleRequest(newRequest("GET", subject, nil), &result); err != nil {
		return nil, err
	}
...
func (c *client) DeleteSubject(subject string, permanent bool) ([]int, error) {
	...
	req := newRequest("DELETE", subjectsDelete, nil, url.PathEscape(subject), permanent)
	if err := c.restService.handleRequest(req, &result); err != nil {
		return nil, err
	}
...
func (c *client) DeleteSubjectVersion(subject string, version int, permanent bool) (int, error) {
	c.versionCacheLock.Lock()
	...
	var result int
	err := c.restService.handleRequest(
		newRequest("DELETE", versionsDelete, nil, url.PathEscape(subject), version, permanent),
		&result,
	)
...
func (c *client) GetCompatibility(subject string) (Compatibility, error) {
	var result compatibilityLevel

	req := newRequest("GET", subjectConfig, nil, url.PathEscape(subject))
	if err := c.restService.handleRequest(req, &result); err != nil {
		return -1, err
	}
...
func (c *client) UpdateCompatibility(subject string, update Compatibility) (Compatibility, error) {
	result := compatibilityLevel{
		CompatibilityUpdate: update,
	}

	req := newRequest("PUT", subjectConfig, &result, url.PathEscape(subject))
	if err := c.restService.handleRequest(req, &result); err != nil {
		return -1, err
	}

	return result.CompatibilityUpdate, nil
}
...
func (c *client) TestCompatibility(subject string, version int, schema SchemaInfo) (bool, error) {
	...
	req := newRequest("POST", compatibility, &candidate, url.PathEscape(subject), version)
	if err := c.restService.handleRequest(req, &result); err != nil {
		return false, err
	}
...
func (c *client) GetDefaultCompatibility() (Compatibility, error) {
	var result compatibilityLevel
	if err := c.restService.handleRequest(newRequest("GET", config, nil), &result); err != nil {
		return -1, err
	}
...
func (c *client) UpdateDefaultCompatibility(update Compatibility) (Compatibility, error) {
	result := compatibilityLevel{
		CompatibilityUpdate: update,
	}

	if err := c.restService.handleRequest(newRequest("PUT", config, &result), &result); err != nil {
		return -1, err
	}
